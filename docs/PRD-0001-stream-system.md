...
### 3.2 Scene 2: Coding (메인 코딩 화면)

![Coding Scene](images/stream-layout-coding-v2.png)

**설계 원칙:**
- 화면 캡처: 사용자가 OBS에서 직접 처리 (Window Capture 등)
- 유튜브 채팅: 유튜브 자체 채팅창 사용
- 오버레이: 우측 패널 + 하단 바 + 캠 영역만 담당
- 모든 영역 16:9 비율 준수
...
│                                              │                    │
│                                              │    우측 패널        │
│    [화면 캡처 영역 - 사용자 OBS 처리]          │    프로젝트 목록    │
│         (1600 x 900)                         │    (320 x 900)     │  900px
│         16:9                                 │                    │
│                                              │    - 현재 작업      │
│                                              │    - TDD 상태       │
│                                              │    - 프로젝트 카드   │
...
|------|-------------|--------------|------|------|
| 화면 캡처 | 0, 0 | 1600 x 900 | 사용자 OBS | 코딩 화면 |
| 우측 패널 | 1600, 0 | 320 x 900 | 오버레이 | 프로젝트 상태 |
| 하단 바 | 0, 900 | 1600 x 180 | 오버레이 | TDD/세션 정보 |
| 얼굴 캠 | 1600, 900 | 320 x 180 | OBS 소스 | 웹캠 |

**우측 패널 구성 (320 x 900) - 이슈 중심:**

| 섹션 | 높이 | 내용 |
|------|------|------|
| 현재 이슈 | 160px | 이슈 번호, 제목, 프로젝트, 브랜치 |
| 진행 단계 | 540px | 이슈 체크리스트 (완료/진행중/대기) |
| TDD 사이클 | 200px | RED/GREEN/REFACTOR 시각화, 테스트 통과율 |

**우측 패널 표시 정보:**
- 현재 작업 중인 이슈만 표시 (여러 프로젝트 목록 제거)
- 이슈의 진행 단계를 체크리스트로 표시
- TDD 사이클 상태를 시각적으로 강조

**하단 바 구성 (1600 x 180) - 세션 타임라인:**

| 요소 | 위치 | 내용 |
|------|------|------|
| LIVE 뱃지 | 좌측 | 빨간 펄스 애니메이션 |
| 세션 타이머 | 좌측 | HH:MM:SS (방송 시간) |
| 타임라인 | 중앙~우측 | 주요 이벤트 시각화 |
